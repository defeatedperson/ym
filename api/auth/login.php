...
// 检查是否已登录（账号JWT令牌验证）
if (isset($_POST['account_jwt'])) {
    $jwt = $_POST['account_jwt'];
    // 这里假设JWT中包含用户名信息，实际可根据你的业务调整
    // 可用 validate_jwt_token 或 verify_account 进行校验
    $login_info = validate_jwt_token($jwt); // 如果你有 validate_jwt_token
    // 或者根据你的实际业务，先解码JWT获取用户名，再用 verify_account 检查
...
if (isset($_POST['action']) && $_POST['action'] === 'verify_slider') {
    ...
    $user_value = isset($_POST['user_value']) ? intval($_POST['user_value']) : 0;
...
/**
 * 登录成功后生成账号JWT和访问JWT令牌
 * @param string $username
 * @return array ['status'=>'ok', 'account_jwt'=>string, 'visit_jwt'=>string]
 */
function generate_login_tokens($username) {
    $account_jwt = generate_jwt_token($username);
    // 获取当前真实IP，用于访问JWT绑定
    $ip = check_ip_status()['ip'];
    $visit_result = generate_visit_jwt($account_jwt, $ip);
    if ($visit_result['status'] !== 'ok') {
        return ['status'=>'jwt_error'];
    }
