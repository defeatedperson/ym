 ...
 *
 * 功能说明：
 * 1. generate_jwt_token($username, $ip)：生成JWT令牌（有效期7天），包含用户ID、用户名、权限信息、IP地址，自动管理密钥。
 * 2. ban_jwt_token($jwt)：将JWT令牌加入强制登出列表（/data/ban-token.json），自动清理过期记录。
 * 3. validate_jwt_token($jwt, $ip)：校验JWT令牌有效性（签名、过期、强制登出、用户信息一致性、IP绑定），返回完整用户信息。
 *
 * 安全特性：
 * - JWT载荷包含用户ID、用户名、管理员标识、IP地址，防止用户名冲突导致的权限混乱和令牌被盗用
 * - 验证时通过用户ID查询数据库，确保JWT信息与实际用户信息一致
 * - IP绑定确保令牌只能在生成它的IP地址上使用，提高安全性
 ...
 * @param string $ip 用户IP地址，用于绑定JWT
 * @return string JWT令牌
 */
function generate_jwt_token($username, $ip) {
    // 首先获取用户的完整信息，包括ID和权限
...
/**
 * 校验JWT令牌是否有效（签名、过期、强制登出、用户信息一致性、IP绑定）
 * 安全特性：
 * - 验证JWT必须包含用户ID、用户名、管理员标识、IP地址
 ...
 */
function validate_jwt_token($jwt, $ip) {
    $key = _get_jwt_secret();
    $parts = explode('.', $jwt);
    ...
    // 验证JWT载荷必须包含完整的用户信息
    if (!isset($payload['sub']) || !isset($payload['uid']) || !isset($payload['adm']) || !isset($payload['ip'])) {
        return ['valid'=>false, 'reason'=>'invalid_payload', 'username'=>null, 'user_id'=>null, 'is_admin'=>false];
    }
    
    // 验证IP地址是否匹配
    if ($payload['ip'] !== $ip) {
        return ['valid'=>false, 'reason'=>'ip_mismatch', 'username'=>null, 'user_id'=>null, 'is_admin'=>false];
    }
